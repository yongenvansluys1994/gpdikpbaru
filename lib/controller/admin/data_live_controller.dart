...
class Live_Controller extends GetxController {
  final _page = 1.obs;
  final _items = <ModeldataLive>[].obs;
  ...
  List<ModeldataLive> get items => _items.toList();
  ...
  ModeldataLive item(int index) => _items[index];
  ...
  Future<void> loadPerPage() async {
    if (!isLoading) {
      ...
      final items = await ApiAuth.datalive(page);
