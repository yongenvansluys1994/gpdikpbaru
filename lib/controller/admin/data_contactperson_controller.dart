...
class ContactPerson_Controller extends GetxController {
  final _page = 1.obs;
  final _items = <ModelContactPerson>[].obs;
  final _isEmpty = false.obs;
  final _isFailed = false.obs;
  final _isLoading = false.obs;
  final _hasReachedMax = false.obs;

  ContactPerson_Controller();

  int get page => _page.value;
  int get count => _items.length;
  List<ModelContactPerson> get items => _items.toList();
  ...
  ModelContactPerson item(int index) => _items[index];
  ...
  Future<void> loadPerPage() async {
    if (!isLoading) {
      ...
      final items = await ApiAuth.datacontactperson(page);
