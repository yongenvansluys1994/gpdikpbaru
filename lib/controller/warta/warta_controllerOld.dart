...
Geser ke kiri untuk melihat warta minggu sebelumnya.
  ...
  void scrollToCurrentWeek() {
    DateTime now = DateTime.now(); // 31 Maret 2025

    // Cari indeks minggu yang mengandung tanggal saat ini
    int currentWeekIndex = wartaItems.indexWhere((item) {
      DateTime startDate = item['startDate'];
      DateTime endDate = item['endDate'];

      // Periksa apakah tanggal saat ini berada dalam rentang minggu ini
      return now.isAfter(startDate.subtract(Duration(days: 1))) &&
          now.isBefore(endDate.add(Duration(days: 1)));
    });

    if (currentWeekIndex != -1) {
      currentPage.value = currentWeekIndex;
      print(
          'Minggu aktif: ${wartaItems[currentWeekIndex]['startDate']} - ${wartaItems[currentWeekIndex]['endDate']}');
    } else {
      // Jika minggu tidak ditemukan, cari minggu terdekat ke depan
      currentWeekIndex = wartaItems.indexWhere((item) {
        DateTime startDate = item['startDate'];
      ...
      if (currentWeekIndex != -1) {
        currentPage.value = currentWeekIndex;
        print(
            'Minggu aktif (terdekat ke depan): ${wartaItems[currentWeekIndex]['startDate']} - ${wartaItems[currentWeekIndex]['endDate']}');
      } else {
        // Jika tidak ada minggu ke depan, tampilkan minggu terakhir yang tersedia
        currentPage.value = wartaItems.length - 1;
        print(
            'Minggu aktif (terakhir): ${wartaItems[currentPage.value]['startDate']} - ${wartaItems[currentPage.value]['endDate']}');
      }
    }
  }
